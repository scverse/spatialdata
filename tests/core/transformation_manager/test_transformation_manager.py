...
def test_remove_coordinate_system_with_associated_transformations(fully_connected_two_point_graph):
    ...
    tm.add_transformation_edge(transformation)
...
def test_add_transformation(fully_connected_two_point_graph):
    ...
    tm = TransformationGraph()
    [cs1, cs2], [transform_edge] = fully_connected_two_point_graph

    tm.add_coordinate_system(cs1)
    tm.add_coordinate_system(cs2)

    tm.add_transformation_edge(transform_edge)

    assert tm._graph.has_edge(cs1, cs2)
    # Get the first edge key and check the transformation
    edge_key = tm._get_edge_key(transform_edge)
    assert tm._graph[cs1][cs2][edge_key][TRANSFORM_KEY] is transform_edge
...
def test_get_direct_transformations(fully_connected_two_point_graph):
    ...
    assert retrieved[0] is transform_edge
...
def test_remove_specific_transformation_between_coordinate_systems(five_point_graph):
    ...
    coord_systems, transform_edges = five_point_graph
    [_cs1, _cs2, cs3, _cs4, cs5] = coord_systems
    [_transform1, _transform2, _transform3, _transform4, _transform5, transform6, transform7] = transform_edges
    tm.add_coordinate_system(cs3)
    tm.add_coordinate_system(cs5)

    tm.add_transformation_edge(transform6)
    tm.add_transformation_edge(transform7)

    tm.remove_specific_transformation(transform7)
    edge_key = tm._get_edge_key(transform7)
...
def assert_sequences_equivalent(
    ...
    assert all(isinstance(x, SequenceEdge) for x in result_sequences)
    transformation_sequences_found = [x.transformations for x in result_sequences]
    assert set(map(tuple, expected_transform_lists)) == set(map(tuple, transformation_sequences_found))
...
def test_get_all_shortest_transformation_sequences_direct(four_point_graph):
    ...
    tm.add_transformation_edge(transform1)
    tm.add_transformation_edge(transform2)
    tm.add_transformation_edge(transform4)

    sequences = tm.get_all_shortest_transformation_sequences(cs1, cs3)
    assert_sequences_equivalent(sequences, [[transform4]])
...
def test_get_all_shortest_transformation_sequences_indirect_one_path(four_point_graph):
    ...
    sequences = tm.get_all_shortest_transformation_sequences(cs1, cs3)
    assert_sequences_equivalent(sequences, [[transform1, transform2]])
...
def test_get_all_shortest_transformation_sequences_indirect_two_paths(five_point_graph):
    ...
    coord_systems, transform_edges = five_point_graph
    [cs1, cs2, cs3, cs4, _cs5] = coord_systems
    [transform1, transform2, transform3, transform4, _transform5, _transform6, _transform7] = transform_edges
    ...
    tm.add_transformation_edge(transform1)
    tm.add_transformation_edge(transform3)
    tm.add_transformation_edge(transform4)
    tm.add_transformation_edge(transform2)

    sequences = tm.get_all_shortest_transformation_sequences(cs1, cs3)
    assert_sequences_equivalent(sequences, [[transform4, transform2], [transform1, transform3]])
...
def test_get_all_shortest_transformation_sequences_multiple_paths_multiple_edges_success(five_point_graph):
    ...
    coord_systems, transform_edges = five_point_graph
    [cs1, cs2, cs3, cs4, cs5] = coord_systems
    [transform1, transform2, transform3, transform4, _transform5, transform6, transform7] = transform_edges
    ...
    expected_intermediate_edges = [transform6]

    sequences = tm.get_all_shortest_transformation_sequences(
        cs1, cs5, expected_intermediate_edges=expected_intermediate_edges
    )
    assert_sequences_equivalent(sequences, [[transform1, transform3, transform6], [transform4, transform2, transform6]])
...
def test_get_all_transformation_sequences(four_point_graph):
    ...
    assert_sequences_equivalent(sequences, [[transform1, transform2, transform3], [transform4, transform3]])
...
def test_get_all_transformation_sequences_multiple_paths(five_point_graph):
    ...
    coord_systems, transform_edges = five_point_graph
    [cs1, cs2, cs3, cs4, cs5] = coord_systems
    [transform1, transform2, transform3, transform4, transform5, _transform6, _transform7] = transform_edges
    ...
    tm.add_transformation_edge(transform4)
    tm.add_transformation_edge(transform2)
    tm.add_transformation_edge(transform5)

    sequences = tm.get_all_transformation_sequences(cs1, cs3)
    assert_sequences_equivalent(sequences, [[transform1, transform3], [transform4, transform2], [transform5]])
...
def test_get_all_transformation_sequences_multiple_paths_multiple_edges_success(five_point_graph):
    ...
    coord_systems, transform_edges = five_point_graph
    [cs1, cs2, cs3, cs4, cs5] = coord_systems
    [transform1, transform2, transform3, transform4, transform5, transform6, transform7] = transform_edges
    ...
    assert_sequences_equivalent(
        sequences,
        [[transform1, transform3, transform6], [transform4, transform2, transform6], [transform5, transform6]],
    )
...
def test_get_all_transformation_sequences_multiple_paths_multiple_edges_multiple_expected(five_point_graph):
    ...
    expected_intermediate_edges = [transform6, transform7]
...
def test_repr_method_comprehensive(fully_connected_two_point_graph):
    ...
    assert repr_str.find(repr(transform_edge))
