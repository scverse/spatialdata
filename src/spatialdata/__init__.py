...
# Setting `dataframe.query-planning` to False is effective only if run before `dask.dataframe` is initialized. In the case
# in which the user had initilized `dask.dataframe` before, we would have DASK_EXPER_ENABLED set to `True`. Here we check
