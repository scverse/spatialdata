...
-   _SpatialData_ is not a new format. Instead, _SpatialData_ builds upon the OME-NGFF.
...
-   [ ] P0. _Raw data IO_: we are implementing readers for raw data of common spatial omics technologies @: scverse/spatialdata-io . The goal is to provide initial readers but we don't plan to perform long-term maintenance, community contribution will be required. Matrix for technologies supported.
...
_SpatialData_ closely follows the OME-NGFF specifications and therefore much of its assumptions are inherited from it. The key assumptions are the following:
...
Transforms map Elements between coordinate systems. Each `Transform` object must have both a source and a destination coordinate system defined. The _SpatialData_ library will implement a subset of

### Examples

#### Layout

Below are some examples of how common spatial datasets would be stored and loaded by _SpatialData_. More examples are available in `https://github.com/giovp/spatialdata-sandbox/`.
